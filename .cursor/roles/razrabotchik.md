...
Следуй `bugfix-protocol.mdc` **до первой правки**:

1. Repro  
2. Логика (store/actions)  
3. DOM (слои, pointer-events, hit-area)  
4. CSS (stacking, overflow, visibility)  
5. Данные save/hydrate  
6. Зафиксировать root cause письменно  
7. Минимальный fix + регрессионный тест (если тестировщик ещё не добавил — добавь тест сам **до** или **сразу с** фиксом, не после «авось»)

**Запрещено** чинить по догадке. Нет доказанного cause — верни оркестратору запрос на уточнение / логи, не патч.

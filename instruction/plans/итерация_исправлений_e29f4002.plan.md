...
overview: "План реализации итерации из instruction/plans/plan.md: фиксы сундука и наград, удаление сметаны, книга бестиария, Суседко, реклама за энергию, настройки в профиле. Ключи к духам и запасные ключи — остаются в каноне."
todos:
  ...
  - id: remove-spare-keys
    content: "ОТМЕНЕНО: ключи к духам (spirit_key) и запасные ключи (spareChestKeys) остаются — не удалять"
    status: cancelled
...
4. Аналогично в [miracleChestLoot.ts](src/domain/miracleChestLoot.ts) — дубликат epic-скина: ре-ролл **или** запасной ключ к обычному сундуку (канон `scenario.md` §5.4).
5. Тесты: [chestLoot.test.ts](src/domain/chestLoot.test.ts), [GameStore.test.ts](src/store/GameStore.test.ts).

---

## Epic 2 — Ключи в сундуке (отмена удаления, решение владельца 2026-09-07)

**Решение:** ключи **остаются** в игре и в каноне.

| Тип | Где | Эффект |
|-----|-----|--------|
| **Ключ к духу** (`spirit_key`) | Обычный сундук (rare/epic), утешительный пул Сундука чудес | Открывает квест Полудницы / Топтыгина / Русалки |
| **Ключ к обычному сундуку** (`spareChestKeys`) | Сундук чудес (утешительный пул), дубликат epic-скина | Сундук 3 ч сразу готов; если готов — +1 запасной (cap уточнить) |

Не удалять из `lootTables.ts`, `chestLoot.ts`, `miracleChestLoot.ts`, `GameSave` / миграции.
...
| 1 | 5 Суседко | P0 | — |
| 2 | 1+6 Сундук | P0 | — |
| 3 | 3 Сметана + over-cap | P1 | — |
| 4 | 4 Книга | P2 | Epic 3 для текстов 2.3 |
| 5 | 6 Реклама за энергию | P2 | Epic 3 (`addRewardEnergy`) |
| 6 | 7 Настройки + облако | P2 | — |

---

## Критерии приёмки (сводка)

- [x] Сундук: название скина на выбранном языке; скин разблокирован, но **не надет** до профиля
- [x] Дубликат скина → автоматический ре-ролл; нет consolation; дефолтные скины не выпадают
- [x] Ключи к духам (`spirit_key`) и запасные ключи (`spareChestKeys`) **не удалены** — в пуле сундуков по канону
- [x] Сметана нигде в UI/луте; +50/+100 энергии напрямую
- [x] Награда может дать 150/100; реген не превышает max
- [x] Книга закрывается только по ✕; победа в квесте — 3D-иллюстрация; тексты наград без «не выше макс.»
- [x] Суседко: warning → stealing → монеты −1 каждые 3 с
- [x] При 0 энергии клик по коту → модалка рекламы +50
- [x] Настройки (язык, облако, сброс) — последняя вкладка профиля; нет панели справа снизу
- [x] `npm test` зелёный
...
Обновить канон по факту: [list_of_spirits.md](instruction/list_of_spirits.md), [loot_tables.md](instruction/dev/loot_tables.md), [scenario.md](instruction/scenario.md) §5.4 — **синхронизировано 2026-09-07**.

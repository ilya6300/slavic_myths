...
- **Критерии приёмки:**
  ...
  - [x] `assetRegistry.ts`: `bookOpen: fromAssets('furniture/book_of_spirits_open.png')`

  - [x] `instruction/assets_catalog.md`: `book_of_spirits_open.png` → статус ✅

  - [x] `instruction/design_assets_prompts.md`: промпты для `book_quest_btn_wood.png`, `icon_paw.png`, `book_close_wood.png` (текст из плана §0.1)

  - [x] Стрелки `arrow_left_wood` / `arrow_right_wood` — промпты в реестре не дублировать (уже есть)

  - [x] Заготовка `bookUi` namespace в `assetRegistry.ts` с путями P0-кнопок (файлы могут отсутствовать до TASK-012)
...
- **Критерии приёмки:**
  ...
  - [x] `assets/ui/arrow_left_wood.png` — деревянная стрелка влево (промпт §`arrow_left_wood`)

  - [x] `assets/ui/arrow_right_wood.png` — деревянная стрелка вправо (промпт §`arrow_right_wood`)

  - [x] `assets/ui/book_quest_btn_wood.png` — плашка «В путь» (промпт плана §0.1)

  - [x] `assets/ui/icon_paw.png` — иконка лапки (промпт плана §0.1)

  - [x] `assets/ui/book_close_wood.png` — кнопка закрыть × (промпт плана §0.1)

  - [x] Все пути подключены в `assetRegistry.ts` (`bookUi` namespace, см. план §0.1)

  - [x] Визуально: стиль clay-wood, читаемость touch 44×44 px (стрелки min 48×48)

  - [x] `instruction/assets_catalog.md`: статус ✅ для каждого файла
...
- **Критерии приёмки:**
  ...
  - [x] Hover на `.scene-book.scene-sprite--interactive:hover .scene-sprite__img`: `scale(1.04)`, `translateY(-3px)`, золотой `drop-shadow` pulse 1.6 s (§1.1)

  - [x] При hover idle opacity-pulse остановлен

  - [x] `@media (prefers-reduced-motion: reduce)` — без scale/glow, только opacity-pulse

  - [x] Нет `width`/`height` на `.scene-book` **для hit-area** (масштаб hi-res — TASK-019, `width: 9vw`)

  - [x] `IzbaSceneLayers.tsx`: класс `scene-sprite--interactive` при `bookAllowed`
...
- **Критерии приёмки:**
  ...
  - [x] Фазы: `idle` → `flying` → `crossfading` → `content` → `closing` → `idle` (диаграмма плана)

  - [x] `startOpen(spiritId, fromRect)`: сохраняет rect, t=1000 ms → crossfade, t=1250 ms → `showPageContent=true`

  - [x] `close()`: цепочка закрытия, сброс таймеров, phase → `idle`

  - [x] `isOverlayActive` = phase ≠ `idle`; `isInteractionBlocked` = phase ≠ `content`

  - [x] `prefers-reduced-motion`: сразу `content` + `coverVariant='open'`

  - [x] `src/store/bookUiStore.test.ts`: переходы фаз, сброс при close, reduced-motion
...
- **Критерии приёмки:**
  ...
  - [x] `IzbaSceneLayers`: `useRef` на `.scene-book`; `handleBookClick` → `getBoundingClientRect()` → `startOpen`; guard при `isOverlayActive`

  - [x] DOM: `.book-flight` (closed) + `.book-modal__stage` (open); `data-phase` на корне

  - [x] FLIP: inline `style` только transform на `.book-flight`; transition 0.5 s `cubic-bezier(0.22, 1, 0.36, 1)`

  - [x] t=1000 ms: crossfade closed → open; t=1250 ms: fade-in spread

  - [x] Пока `isOverlayActive`: `.scene-book { visibility: hidden }` на сцене

  - [x] `useOnboardingBootstrap`: pan-block при `isOverlayActive` (не только `isOpen`)

  - [x] `grep style=` в book-модулях — только FLIP transform
...
- **Критерии приёмки:**
  ...
  - [x] Удалены `.book-modal__inner`, дублирующий заголовок «Тайны славянских духов», `cover-open` с opacity 0.12 / `bookClosed`

  - [x] `.book-modal__stage`: размеры по TASK-019 (`BOOK_OPEN_ASPECT` 1.248; **не** `/ 1.55`)

  - [x] Landscape `max-height: 500px`: высота `86vh`, ширина `86vh * 1.248` (TASK-019)

  - [x] Зоны контента по §2.6: ribbon, left/right page, bookmark, close, nav

  - [x] `.book-page` без `background: rgba(...)`; текст `#3d2a1a`, имя `#8b6914`

  - [x] Иллюстрация: `max-height: 58%` правой страницы (не 180px); landscape 62%

  - [x] Левая страница: `overflow-y: auto`

  - [x] Стрелки: `arrow_*_wood.png` min 48×48; «В путь»: `book_quest_btn_wood` + `icon_paw`; закрыть: `book_close_wood` — или CSS-fallback до TASK-012

  - [x] `aria-labelledby` → `book-page__name`; контент скрыт до `showPageContent`

  - [x] Закладка: CSS-лента `#3d7a4a` до P1-ассета; грейд — CSS-классы до `badge_grade_*.png` (P1)
...
- **Критерии приёмки:**
  ...
  - [x] Суммарная длительность закрытия ≤500 ms (§2.3)

  - [x] Последовательность: fade-out UI (0.15 s) → crossfade (0.15 s) → FLIP назад (0.2 s) → phase `idle`

  - [x] Клик backdrop и кнопка × не прерывают полёт/crossfade — только в `content`

  - [x] После закрытия книга на сцене снова в idle-pulse

  - [x] `prefers-reduced-motion`: мгновенное закрытие
...
- **Критерии приёмки:**
  ...
  - [x] Перелистывание: rotateY 0°→−90°→swap→90°→0°, суммарно 0.35–0.45 s (§3.2)

  - [x] `perspective` на `.book-modal__spread`; `.book-page--flipping` на уходящей странице

  - [x] Свайп горизонталь >40 px по `.book-modal__spread`

  - [x] Во время flip: «В путь» и стрелки disabled; быстрые тапы — очередь из 1

  - [x] `prefers-reduced-motion`: мгновенная смена разворота

  - [x] Стрелки скрыты на первой/последней странице (scenario.md §5.1)
...
- **Критерии приёмки:**
  ...
  - [x] Скрин vs mockup по зонам: центр левой страницы, компактная CTA, нить по центру, 3D Домовой (код + CSS-контракт зелёные; живой overlay — visual_check владельца: клик по книге после онбординга)
...
- **Критерии приёмки:**
  ...
  - [x] Visual: иллюстрация по центру правой страницы, `contain`, как mockup v2 (dev check владельца)
...
- **Критерии приёмки:**
  - [x] `cat_pilgrim` sid+sleep в `assets/pets/rare/cat_pilgrim/`
  - [x] `starter_casket.png`, `yard_grass.png`, `kikimora_craft_bg.jpeg`, `kikimora_weaving.png`
  - [x] `icon_energy.png`, `icon_smetana.png` в `assets/UI/`
  - [x] Mockups: `starter_pack_mockup.png`, `street_yard_mockup.png`, `kikimora_craft_mockup.png`, `rewarded_wait_mockup.png`
  - [x] UX-спеки с таблицей Layout vs mockup; `assets_catalog.md` обновлён
...
- **Критерии приёмки:**
  - [x] `instruction/dev/tech.md` §Epic 13
  - [x] `street_layout.md` черновик позиций
- **Вне scope:** код
...
- **Критерии приёмки:**
  - [x] `energy===0` → sleep, клик без монеты
  - [x] AFK sleep → клик будит, монета
  - [x] Суседко не стартует при tired
  - [x] 1–2 клика tired, затем EnergyRewardModal
  - [x] Онбординг: tired выключен (floor 20)
...
- **Критерии приёмки:**
  - [x] Все тесты TASK-030 зелёные
  - [x] Банк `tired` (5 реплик ru/en/tr)
...
- **Критерии приёмки:**
  - [x] Dev: ≥10 с до onRewarded, виден отсчёт
  - [x] Отмена <3 с без награды
  - [x] Тест: stub не вызывает onRewarded синхронно
  - [x] Миска сметаны + ускорение сундука
...
- **Критерии приёмки:**
  - [x] SAVE v9, миграция
  - [x] +100 энергии сверх max, +5 оберегов, скин `cat_pilgrim`
  - [x] Повтор: «Ларец уже твой»
  - [x] `cat_pilgrim` не в skinPools сундука
  - [x] Точка входа после первого сундука
...
- **Критерии приёмки:**
  - [x] 3 травы → 1 оберег, лимит суток
  - [x] Тесты домена зелёные
...
- **Критерии приёмки:**
  - [x] Pan street←1→2; дефолт комната 1 визуально как эталон
  - [x] Улица без `.layer-izba`, без кликера монет
  - [x] Аватар Кикиморы после defeated → модалка крафта
  - [x] Банки `yard_blocked_zhirdyay` (10), `yard_blocked_night`
  - [x] Комната 1 CSS не сломана
...
- **Критерии приёмки:**
  - [x] Трофей внутри рамки в TrophyModal
  - [x] poludnik — тот же размер что остальные
  - [x] Статистики на всю ширину правой колонки
  - [x] 7 вкладок в один ряд на всю ширину
...
- **Критерии приёмки:**
  - [x] Все чекбоксы plan.md закрыты или эскалированы
  - [x] Комната 1 эталон не сломана
  - [x] `npm test` зелёный
...
- **Критерии приёмки:**
  ...
  - [x] Ночь: подписи читаются на `.layer-night` (выемка, не тонкий текст на брёвнах)
  - [x] Pan 1→2: пол-лейблы не уезжают и не дублируются в комнате 2
  - [x] Оба сундука сразу: таймер + бар не слипаются, ковёр кота не перекрыт
  - [x] Края бара: `0/700` (пустая заливка) и `700/700` (заливка до края + пульс спрайта)
...
- **Критерии приёмки:**
  - [x] Описана отдельная от `GameSkins` extensible-модель: список принадлежащих эффектов и один выбранный `effectId | null`; она не использует `ownedSkinIds` и не попадает в skin pools.
  - [x] Зафиксирован единственный стартовый эффект этой итерации — «Грозовая изба» / `thunder_izba`: победа над Перуном добавляет владение ровно один раз и не меняет выбранный эффект автоматически.
  - [x] Зафиксированы инварианты: допускается ровно один выбранный owned-эффект либо none; нельзя выбрать неполученный; снятие эффекта сохраняет none; старые сейвы мигрируют в пустое владение + none.
  - [x] Описаны границы существующих систем: порядок `chudo_yudo → yarilo → perun`; требования фрагментов 8/10; оба трофея; эффект затемняет обе комнаты и даёт периодическую молнию.
  - [x] FX-контракт помещает некликабельный слой выше `.layer-night`, ниже `.layer-fx`/HUD, сохраняет pan и hit-area; `prefers-reduced-motion` оставляет статичное затемнение без вспышек.
  - [x] Для комнаты трофеев зафиксирована потребность в 17 слотах: натуральные размеры PNG, не более двух трофеев на полку, без изменения эталонных стилей комнаты 1.
- **Проверка:** n/a
...
- **Критерии приёмки:**
  - [x] `profile_layout.md` или связанная UX-спека описывает отдельную вкладку «Атмосфера»: карточку «Без эффекта», locked «Грозовую избу» с подсказкой «Победите Перуна», owned «Выбрать» и equipped «Выбрано».
  - [x] Спека подтверждает, что в профильной вкладке одновременно выбрана только одна атмосфера либо none, а победа Перуна не включает эффект сама.
  - [x] Зафиксированы preview и visual-check для мягкого затемнения плюс заметной периодической молнии в комнатах 1 и 2; reduced motion — только статичное затемнение.
  - [x] Раскладка поддерживает 17 трофеев, включая золотой щит Яровита и дубовый щит громовержца; на полке максимум два трофея, у `.room-shelf`/`__img` нет width, размеры спрайтов натуральные.
  - [x] Спека не добавляет gradient/отдельный фон в комнату 2, сохраняет `WindowAperture` до `.layer-izba` и не меняет эталон комнаты 1.
  - [x] Описаны точные asset paths и визуальные fallback для портретов, иллюстраций и трофеев до появления PNG.
...
- **Критерии приёмки:**
  ...
  - [x] Ассеты визуально соответствуют утверждённой UX-спеке, читаемы в книге и на полках при натуральном размере.
  - [x] `assets_catalog.md` и промпты обновлены с фактическими путями и статусами.
  - [x] Финальный asset visual-check на билде подтверждён владельцем или ревьювером.
- **Проверка:** visual_check
...
- **Критерии приёмки:**
  - [x] Тесты требуют 18 духов в порядке `chudo_yudo → yarilo → perun`, unlock по 8/10 фрагментов и доступность новых locked-целей для текущего дропа без пересмотра шансов.
  - [x] Тесты требуют награду Ярилы (титул + трофей) и награду Перуна (трофей + `thunder_izba` ownership ровно раз, без auto-equip).
  - [x] Тесты миграции требуют пустой список атмосфер и none для существующих сохранений; тесты store запрещают выбрать неполученную атмосферу и подтверждают снятие выбранной.
  - [x] Тесты profile/DOM требуют вкладку «Атмосфера», карточку none и состояния locked/owned/equipped; финальные PNG заменяемы fallback без падения.
  - [x] Тесты CSS/DOM требуют некликабельный FX-слой в обеих комнатах выше night и ниже gameplay FX/HUD, reduced-motion без анимации, и 17 трофеев без width на полках/спрайтах и более двух трофеев на полку.
- **Проверка:** test
...
- **Критерии приёмки:**
  - [x] Ярило и Перун находятся сразу после Чудо-Юдо во всех runtime-источниках порядка; общее количество духов — 18.
  - [x] Требования фрагментов равны 8 для Ярилы и 10 для Перуна, а выбор цели фрагмента охватывает их без hardcoded-списка из 16 духов.
  - [x] Использованы принятые владельцем вопросы, награды, описания, трофеи и локации без правки канонических документов.
  - [x] Реестры используют согласованные пути с безопасными fallback, когда финальных PNG нет; генерация или подмена ассетов не выполняется.
  - [x] Целевые тесты TASK-042 для прогрессии, викторин и фрагментов зелёные.
- **Проверка:** test
- **Следующий:** razrabotchik
...
- **Критерии приёмки:**
  - [x] Модель атмосфер отделена от skins и расширяема каталогом, но содержит только `thunder_izba` в этой итерации.
  - [x] Победа Перуна добавляет owned-эффект идемпотентно и оставляет выбранный эффект none/прежним значением — auto-equip отсутствует.
  - [x] `equipIzbaEffect(effectId | null)` принимает только owned id или none, сохраняет результат и не позволяет экипировать locked эффект.
  - [x] Миграция старых сохранений создаёт пустое ownership и none, не меняя старые скины, очереди и прогресс.
  - [x] Все test-критерии TASK-042 для migration/reward/store зелёные.
- **Проверка:** test
- **Следующий:** razrabotchik
...
- **Критерии приёмки:**
  - [x] Профиль содержит отдельную «Атмосферу» наряду с существующими вкладками; «Без эффекта» доступен всегда.
  - [x] `thunder_izba` до победы Перуна показан locked с «Победите Перуна» и без кнопки выбора; после победы доступен «Выбрать»/«Выбрано».
  - [x] Одновременно выбран только один owned-эффект либо none; выбор/снятие мгновенно обновляют preview и переживают reload.
  - [x] Награда Перуна не открывает и не выбирает вкладку/эффект автоматически.
  - [x] UI работает с fallback-данными, пока TASK-041 blocked; отсутствующий PNG не ломает модалку.
- **Проверка:** test
- **Следующий:** razrabotchik
...
- **Критерии приёмки:**
  - [x] Эффект рендерится только при выбранном `thunder_izba`; владение без выбора не меняет сцену.
  - [x] Обе комнаты получают мягкое затемнение и периодическую заметную молнию, не меняя `skins.izba` и не создавая отдельный фон комнаты 2.
  - [x] FX-слой `pointer-events: none`, находится выше `.layer-night`, ниже `.layer-fx`/HUD и не блокирует кота, сундуки, трофеи, профиль или pan.
  - [x] `prefers-reduced-motion` сохраняет затемнение, но исключает вспышки.
  - [x] В каждой комнате сохранены `WindowAperture` перед `.layer-izba`, одинаковый скин избы и канонический стек; эталонные классы комнаты 1 не изменены.
- **Проверка:** visual_check
- **Следующий:** razrabotchik
...
- **Критерии приёмки:**
  - [x] После побед Ярило открывает золотой щит Яровита, Перун — дубовый щит громовержца; оба имеют fallback до TASK-041.
  - [x] В комнате доступны 17 трофеев; на каждой полке максимум два, а трофей сохраняет натуральный размер.
  - [x] На `.room-shelf` и `.room-shelf__img` нет `width`; не используются `%` для размеров/позиций scene/room-спрайтов без решения владельца.
  - [x] Полки остаются слева/справа от окна, без растягивания на весь экран; пустые слоты и модалки трофеев сохраняют прежнее поведение.
  - [x] Комната 2 использует `.layer-izba` с тем же `resolveHouseUrl(skins.izba)`, `WindowAperture` до неё, без gradient/отдельного trophy-room background.
- **Проверка:** visual_check
- **Следующий:** revyuzer
...
- **Критерии приёмки:**
  - [x] В книге Ярило и Перун следуют непосредственно за Чудо-Юдо; 8/10 фрагментов, авторский контент и обе награды соответствуют принятому канону.
  - [x] Победа Перуна выдаёт «Грозовую избу» во владение без auto-equip; профиль допускает ровно один effect либо none и запрещает locked selection.
  - [x] 17 трофеев, включая оба щита, читаемы на полках: максимум два на полку, натуральные размеры, room-2 scene rules сохранены.
  - [x] «Грозовая изба» проверена в комнатах 1 и 2, днём и ночью: затемнение/молния видимы, reduced motion без вспышек, клики и pan не блокируются.
  - [x] Финальные ассеты TASK-041 визуально приняты; fallback не выдан за финальную asset-приёмку.
  - [x] Целевые тесты, полный `npm test` и `npm run build` зелёные; `review_verdict.status: approved` записан.
- **Проверка:** visual_check
...
- **Критерии приёмки:**
  ...
  - [x] Epic 15 TASK-048 закрыт (`epic15_review.md`)
...
> **Решение владельца (Илья), 2026-09-25.** Перебивает в узком месте: `scenario.md` §6.1 «Rewarded −30 мин», `scenario.md` §7 текст кнопки «−30 мин», `room_01_layout.md` §6 тот же текст, `draft.md` §1 «календаря взгляда нет» — только для одного взгляда без свечи. Базовый кулдаун сундука **3 часа** (`scenario.md` §6.1) не меняется. Свеча по-прежнему открывает взгляд (`draft.md` §1). Одна ежа в календарный день и запрет догона (`draft.md` §2) остаются; рекламный сброс — единственное исключение, один раз после награды.
>
> Имена полей сейва и модули — в `tech.md` (архитектор). Здесь только правила владельца.
>
> Три механики друг от друга не зависят. Внутри механики UI ждёт домен. Первая в работу — TASK-056.

### TASK-056 — Домен и сейв лимита rewarded сундука
- **Статус:** todo
- **Приоритет:** P1
- **Зависит от:** —
- **Канон:** `scenario.md` §6.1 (базовые 3 часа; −30 мин перебито владельцем 2026-09-25); `scenario.md` §7; `room_01_layout.md` §6
- **Цель игрока:** Ускорить обычный сундук рекламой: один ролик снимает 1,5 часа, таких роликов четыре, потом кулдаун 3 часа идёт сам.
- **Суть:** Скип успешного rewarded сундука — 90 минут и −1 заряд из 4; заряд и восстановление лимита живут в сейве. Базовые 3 часа кулдауна не меняются.
- **Scope:** успешный rewarded обычного сундука; заряд 4; миграция старого сейва; изоляция счётчика от других роликов.
- **Критерии приёмки:**
  - [ ] Один успешный rewarded сундука снимает **1,5 часа** (90 минут), не 30 минут
  - [ ] Можно посмотреть рекламу **4 раза**: это два полных открытия сундука, 2 просмотра × 1,5 ч = 3 ч кулдауна
  - [ ] После 4 просмотров следующий кулдаун сундука идёт живые 3 часа без рекламы
  - [ ] Когда эти 3 часа реального ожидания прошли и сундук снова готов без скипа, лимит снова **4**
  - [ ] Пока заряды не исчерпаны и таймер истёк сам, лимит не пополняется и не сгорает
  - [ ] Заряд тратится только на успешный rewarded сундука; отмена или неуспех ролика не снимает время и не тратит заряд
  - [ ] Энергия и другие ролики этот счётчик не трогают
  - [ ] Если до конца таймера меньше 1,5 ч, один успешный просмотр стоит 1 заряд и доводит сундук до готовности, не уходит в минус за готовность
  - [ ] Лимит хранится в сейве и не сбрасывается при перезагрузке
  - [ ] Старые сейвы без новых полей мигрируют к полному лимиту 4, без потери прогресса
  - [ ] Базовый кулдаун открытия остаётся 3 часа
- **Проверка:** test
- **Следующий:** tehnicheskiy-arhitektor
- **Вне scope:** interstitial; реклама на клик кота; смена базовых 3 часов кулдауна; сундук чудес; текст и видимость кнопки (TASK-057)

### TASK-057 — UI скипа сундука: кнопка и «1,5 часа»
- **Статус:** todo
- **Приоритет:** P1
- **Зависит от:** TASK-056
- **Канон:** `room_01_layout.md` §6; `scenario.md` §7 — текст «−30 мин» перебит владельцем 2026-09-25
- **Цель игрока:** Пока есть заряды, видеть кнопку скипа и понимать, что ролик снимает 1,5 часа; при нуле зарядов кнопки нет, таймер идёт сам.
- **Суть:** Кнопка скипа обычного сундука видна только при зарядах > 0. Надпись показывает 1,5 часа, не 30 минут. Точную реплику задаёт дизайнер (или сценарист по его запросу), не разработчик.
- **Scope:** видимость кнопки скипа обычного сундука; смысл надписи «1,5 часа».
- **Критерии приёмки:**
  - [ ] Пока заряды есть, кнопка скипа видна
  - [ ] При 0 зарядах кнопки скипа нет, таймер 3 ч идёт сам
  - [ ] Текст кнопки показывает 1,5 часа, не 30 минут и не «−30 мин»
  - [ ] То же во всех уже существующих локалях этой кнопки (RU / EN / TR)
  - [ ] Конкретную реплику пишет дизайнер или сценарист; в задаче зафиксирован только смысл «1,5 часа, не 30 минут»
- **Проверка:** test (наличие и отсутствие кнопки и текст «1,5 часа» / не «30 минут» утверждаются тестом UI)
- **Следующий:** game-designer-ui-ux
- **Вне scope:** interstitial; реклама на клик кота; смена базовых 3 часов кулдауна; сундук чудес; новый макет модалки

### TASK-058 — Домен и сейв одного рекламного взгляда в зеркало
- **Статус:** todo
- **Приоритет:** P1
- **Зависит от:** —
- **Канон:** `instruction/plans/draft.md` §1 (свеча и закрытое зеркало до Яги остаются; «календаря нет» перебито только для взгляда без свечи)
- **Цель игрока:** При открытом зеркале и нуле свечей один раз за календарные сутки посмотреть рекламу и провести обычный сеанс, не получая и не тратя свечу.
- **Суть:** Успешный rewarded заменяет одну свечу на один взгляд в локальные сутки ежедневки. Свечи не меняются. Заряды сундука не меняются. Факт взгляда лежит в сейве.
- **Scope:** ветка «0 свечей и зеркало открыто»; суточный лимит 1; сейв и день `getCalendarDayId`.
- **Критерии приёмки:**
  - [ ] Если свечи есть — как сейчас: тратится свеча, реклама не предлагается
  - [ ] Если свечей 0 и зеркало уже открыто (после Яги) — один взгляд за календарные сутки по rewarded
  - [ ] Свеча не начисляется и не списывается: сеанс идёт как обычный
  - [ ] Суточный взгляд списывается в тот же момент, когда обычный сеанс списал бы свечу; «Не сейчас» и отмена ролика сеанс не начинают и день не тратят
  - [ ] Второй взгляд в те же сутки без свечи недоступен ни рекламой, ни иначе
  - [ ] Взгляд со свечой в те же сутки по-прежнему доступен и рекламный лимит не тратит
  - [ ] На следующий календарный день лимит снова 1
  - [ ] День — тот же календарный день, что у ежедневки (локальный day id проекта)
  - [ ] Лимит дня хранится в сейве
  - [ ] Старый сейв без нового поля открывается с доступным суточным взглядом, без потери прогресса
  - [ ] Закрытое зеркало (до Яги) рекламу не предлагает и сеанс не начинает
  - [ ] Этот ролик не тратит заряды сундука
- **Проверка:** test
- **Следующий:** tehnicheskiy-arhitektor
- **Вне scope:** бесплатные свечи; смена цены IAP; второй рекламный взгляд в те же сутки; текст и место кнопки (TASK-059)

### TASK-059 — UI рекламного взгляда в зеркало
- **Статус:** todo
- **Приоритет:** P1
- **Зависит от:** TASK-058
- **Канон:** `instruction/plans/draft.md` §1
- **Цель игрока:** Увидеть предложение ролика только когда зеркало открыто, свечей нет и суточный взгляд ещё не использован.
- **Суть:** Предложение rewarded взгляда есть только в этой ветке. Точную реплику задаёт дизайнер.
- **Scope:** видимость предложения rewarded на открытом зеркале при 0 свечей.
- **Критерии приёмки:**
  - [ ] Если свечи есть, предложение рекламного взгляда не показывается
  - [ ] Если свечей 0, зеркало открыто и суточный взгляд ещё не использован — предложение rewarded видно
  - [ ] Второй взгляд в те же сутки без свечи: предложения нет
  - [ ] Закрытое зеркало (до Яги) рекламу не предлагает
  - [ ] Этот ролик не тратит заряды сундука
- **Проверка:** test (видимость предложения утверждается тестом UI)
- **Следующий:** game-designer-ui-ux
- **Вне scope:** бесплатные свечи; смена цены IAP; второй рекламный взгляд в те же сутки; новый макет сеанса

### TASK-060 — Домен и сейв одного суточного сброса ежедневки
- **Статус:** todo
- **Приоритет:** P1
- **Зависит от:** —
- **Канон:** `instruction/plans/draft.md` §2 (одна ежа в день и запрет догона остаются; один сброс после награды — решение владельца 2026-09-25)
- **Цель игрока:** После полученной награды ежедневки один раз за эти сутки посмотреть рекламу и пройти ту же ежу ещё раз.
- **Суть:** Успешный rewarded после награды дня обнуляет выполнение текущей ежи в том же календарном дне. Второй сброс закрыт. Флаг сброса лежит в сейве и снимается вместе с обычным сбросом дня. Награда повторной ежи — тот же фрагмент по текущим правилам дропа.
- **Scope:** один сброс после награды текущего дня; сейв флага; изоляция от зарядов сундука и взгляда зеркала.
- **Критерии приёмки:**
  - [ ] Если ежедневка этого календарного дня уже завершена (награда дня получена), игрок может один раз за эти сутки посмотреть rewarded и сбросить ежедневку, чтобы пройти её ещё раз в этот же день
  - [ ] Сброс обнуляет пункты текущего дня и флаг награды; календарный день тот же; дух сказа дня не перевыбирается
  - [ ] Второй сброс в те же сутки недоступен
  - [ ] Пропущенные прошлые дни по-прежнему не копятся
  - [ ] Сброс доступен только после завершения текущей ежи, не вместо неё
  - [ ] Сброс недоступен до открытия ежедневки (после Банника)
  - [ ] Отмена или неуспех ролика ежедневку не сбрасывает и флаг не тратит
  - [ ] Награда повторной ежи — та же, что у обычной (фрагмент по текущим правилам дропа)
  - [ ] Флаг «сброс уже использован сегодня» хранится в сейве и сбрасывается на новый календарный день вместе с обычным сбросом ежи
  - [ ] Старый сейв без флага открывается как «сброс сегодня не использован», без потери прогресса ежи
  - [ ] Ролик не тратит заряды сундука и не заменяет взгляд в зеркало
- **Проверка:** test
- **Следующий:** tehnicheskiy-arhitektor
- **Вне scope:** вторая рекламная ежа в те же сутки; догон пропусков; смена награды ежи; новые духи; текст и место кнопки (TASK-061)

### TASK-061 — UI одного суточного сброса ежедневки
- **Статус:** todo
- **Приоритет:** P1
- **Зависит от:** TASK-060
- **Канон:** `instruction/plans/draft.md` §2
- **Цель игрока:** Видеть сброс ежедневки по рекламе только после награды этого дня и только пока суточный сброс не использован.
- **Суть:** Контрол rewarded-сброса виден лишь в этой ветке. Точную надпись задаёт дизайнер.
- **Scope:** видимость сброса на уже открытой ежедневке после награды дня.
- **Критерии приёмки:**
  - [ ] Предложение сброса видно, когда награда этого календарного дня получена и сброс сегодня ещё не использован
  - [ ] До награды дня и вместо незавершённой ежи предложения нет
  - [ ] До открытия ежедневки (до Банника) предложения нет
  - [ ] После использованного сброса в те же сутки предложения нет
  - [ ] Ролик не тратит заряды сундука и не заменяет взгляд в зеркало
- **Проверка:** test (видимость предложения утверждается тестом UI)
- **Следующий:** game-designer-ui-ux
